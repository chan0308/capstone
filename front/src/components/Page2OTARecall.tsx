  ...
  return (
    <div className="max-w-[1800px] mx-auto px-8 py-12">
      ...
      <div className="relative mb-16 overflow-hidden rounded-3xl">
        ...
        >
          <h1 className="text-5xl mb-6 text-black font-bold tracking-tight">
            Background: OTA Recalls and Safety Reliability Challenges
          </h1>
          <div className="flex items-start gap-4 max-w-5xl">
            <div className="w-1.5 h-8 bg-red-600 rounded-full mt-1"></div>
            <p className="text-2xl text-gray-700 leading-relaxed font-medium">
              Tesla's full OTA recall strategy reduces surface-level costs but functions as a Failure-response, not
              Prevention — leaving safety trust unresolved.
            </p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-8">
        {/* Sentiment Visualization + Chatbot (한 카드 안에) */}
        <motion.div
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ delay: 0.3, duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
          className="bg-white/70 backdrop-blur-2xl rounded-3xl border border-gray-200/80 p-8 hover:bg-white/80 transition-all duration-300 shadow-xl hover:shadow-2xl flex flex-col"
        >
          ...
          <div className="mt-6 mb-4 flex items-center justify-center gap-6">
            ...
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-gray-600 shadow-lg shadow-gray-500/40" />
            ...
            <FSDChatAssistant
              onUpdateSentiment={(points) => {
                setSentimentData((prev) =>
                  prev.map((row) => {
                    const updated = points.find((p) => p.topic === row.topic);
                    if (!updated) return row;

                    const s = updated.score;
            ...
            {timelineEvents.map((event, index) => (
              ...
              >
                  ...
                  <div
                    className={`w-3 h-3 rounded-full mt-1.5 shadow-lg ${
                      event.type === 'recall'
                        ...
                        ? 'bg-gray-600 shadow-gray-500/40'
                        : event.type === 'incident'
                        ? 'bg-red-700 shadow-red-600/40'
                ...
                {index < timelineEvents.length - 1 && (
                  <div className="absolute left-[22px] top-full w-0.5 h-5 bg-gradient-to-b from-gray-300 to-transparent" />
                )}
              </motion.div>
            ))}
          </div>
        ...
        <div className="flex items-start gap-5">
          ...
          <div>
            ...
            <p className="text-gray-700 text-base leading-relaxed font-medium">
              OTA recalls are cost-efficient responses to failures, but do not constitute preventative measures. The
              recurring pattern suggests systemic quality issues that require upstream Prevention investment to resolve
              root causes and restore consumer trust.
            </p>
          </div>
        </div>
      </motion.div>
